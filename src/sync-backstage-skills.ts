/**
 * sync-backstage-skills.ts — Sync backstage-development skills from upstream repositories.
 *
 * Sources:
 *   1. https://github.com/vinzscam/backstage-skills (master)
 *   2. https://github.com/backstage/backstage docs/.well-known/skills (master)
 *
 * Skills unique to this repo (e.g. backstage-testing-conventions) are preserved.
 *
 * Usage:
 *   tsx src/sync-backstage-skills.ts                  # sync all
 *   tsx src/sync-backstage-skills.ts --dry-run        # preview
 *   tsx src/sync-backstage-skills.ts --output ci      # emit GitHub Actions outputs
 */
...
const PLUGIN_SKILLS_DIR = join(REPO_ROOT, "plugins", "backstage-development", "skills");

const LOCAL_ONLY_SKILLS = new Set(["backstage-testing-conventions"]);
...
if (args.help) {
  console.log(`Usage: tsx src/sync-backstage-skills.ts [options]
...
const WORK_DIR = mkdtempSync(join(tmpdir(), "sync-backstage-skills-"));
...
// --- Source 1: vinzscam/backstage-skills ---
console.log(c.cyan("Syncing backstage-development skills from upstream..."));
console.log("");
console.log(`${c.cyan("[1/2]")} Cloning vinzscam/backstage-skills...`);

git(`clone --depth 1 --quiet https://github.com/vinzscam/backstage-skills.git ${join(WORK_DIR, "vinzscam")}`);
const vinzscamDir = join(WORK_DIR, "vinzscam", "skills");
const vinzscamSkills = getSkillDirs(vinzscamDir);

syncSource(vinzscamDir, result, processedSkills);
console.log(`  ${c.green("✓")} vinzscam/backstage-skills: ${vinzscamSkills.length} skills`);
console.log("");

// --- Source 2: backstage/backstage official skills ---
console.log(`${c.cyan("[2/2]")} Cloning backstage/backstage (sparse: docs/.well-known/skills)...`);

const backstageClone = join(WORK_DIR, "backstage");
git(`clone --depth 1 --sparse --quiet https://github.com/backstage/backstage.git ${backstageClone}`);
git("sparse-checkout set docs/.well-known/skills", { cwd: backstageClone });

const backstageDir = join(backstageClone, "docs", ".well-known", "skills");
const backstageSkills = getSkillDirs(backstageDir);

syncSource(backstageDir, result, processedSkills);
console.log(`  ${c.green("✓")} backstage/backstage: ${backstageSkills.length} skills`);
console.log("");

// --- Detect removals ---
const allUpstream = new Set([...vinzscamSkills, ...backstageSkills]);
...
if (OUTPUT_MODE === "ci") {
  ...
  if (hasChanges) {
    ...
    const summaryFile = join(process.env.RUNNER_TEMP || tmpdir(), "pr-body.md");
    writeFileSync(summaryFile, prBody);
    setOutput("summary_file", summaryFile);
  }

  // Also print human-readable summary to stdout for CI logs
