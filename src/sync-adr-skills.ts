/**
 * sync-adr-skills.ts — Sync adr plugin skills from upstream repository.
 *
 * Source:
 *   https://github.com/zircote-plugins/adr skills/ (main)
 *
 * Only the portable skills/ are synced. Client-specific parts of the upstream
 * plugin (agents/, commands/, templates/) are not synced; the com.multica
 * agents in this plugin are maintained locally.
 *
 * Skills unique to this repo are preserved and never overwritten.
 *
 * Usage:
 *   tsx src/sync-adr-skills.ts                  # sync all
 *   tsx src/sync-adr-skills.ts --dry-run        # preview
 *   tsx src/sync-adr-skills.ts --output ci      # emit GitHub Actions outputs
 */
...
const PLUGIN_SKILLS_DIR = join(REPO_ROOT, "plugins", "adr", "skills");
...
if (args.help) {
  console.log(`Usage: tsx src/sync-adr-skills.ts [options]
...
const WORK_DIR = mkdtempSync(join(tmpdir(), "sync-adr-skills-"));
...
// --- Source: zircote-plugins/adr skills/ (sparse) ---
console.log(c.cyan("Syncing adr skills from upstream..."));
console.log("");
console.log(`${c.cyan("[1/1]")} Cloning zircote-plugins/adr (sparse: skills)...`);

const adrClone = join(WORK_DIR, "adr");
git(`clone --depth 1 --sparse --quiet https://github.com/zircote-plugins/adr.git ${adrClone}`);
git("sparse-checkout set skills", { cwd: adrClone });

const adrSkillsDir = join(adrClone, "skills");
const adrSkills = getSkillDirs(adrSkillsDir);

syncSource(adrSkillsDir, result, processedSkills);
console.log(`  ${c.green("✓")} zircote-plugins/adr: ${adrSkills.length} skills`);
console.log("");

// --- Detect removals ---
const allUpstream = new Set(adrSkills);
...
if (OUTPUT_MODE === "ci") {
  ...
  if (hasChanges) {
    ...
    const summaryFile = join(process.env.RUNNER_TEMP || tmpdir(), "pr-body-adr.md");
